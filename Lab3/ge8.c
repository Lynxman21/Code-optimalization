                         ...
                         const double *mult)
{
  ...
  for (j = 0; j + NR <= n; j += NR) {
    ...
    register __m256d p23 = _mm256_loadu_pd(pivot_row + j + 2);
    register __m256d p45 = _mm256_loadu_pd(pivot_row + j + 4);
    register __m256d p67 = _mm256_loadu_pd(pivot_row + j + 6);
    ...
    for (i = 0; i < m; i++) {
      ...
      register __m256d c23 = _mm256_loadu_pd(c + 2);
      register __m256d c45 = _mm256_loadu_pd(c + 4);
      register __m256d c67 = _mm256_loadu_pd(c + 6);

      c01 = _mm256_sub_pd(c01, _mm256_mul_pd(p01, mmult));
      c23 = _mm256_sub_pd(c23, _mm256_mul_pd(p23, mmult));
      c45 = _mm256_sub_pd(c45, _mm256_mul_pd(p45, mmult));
      c67 = _mm256_sub_pd(c67, _mm256_mul_pd(p67, mmult));

      _mm256_storeu_pd(c,     c01);
      _mm256_storeu_pd(c + 2, c23);
      _mm256_storeu_pd(c + 4, c45);
      _mm256_storeu_pd(c + 6, c67);
    }
  }
