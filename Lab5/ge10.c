...
static void PackUPanel(int kdim, int n, const double *A, int SIZE, int row0, int col0, double *U){
  int r,j ;
  for (r = 0; r < kdim; r++) {
    for (j = 0; j < n; j++) {
...
static void PackLPanel(int m, int kdim, const double *A, int SIZE, int row0, int col0, double *L){
  int i,r ;
  for (i = 0; i < m; i++) {
    for (r = 0; r < kdim; r++) {
    ...
    for (k = k0; k < kend; k++) {
      ...
      for (jj = k + 1; jj < kend; jj ++) {
        ...
        for (ii = k + 1; ii < SIZE; ii ++) {
          A[IDX(ii, jj, SIZE)] = A[IDX(ii, jj, SIZE)] - pj * (A[IDX(ii, k, SIZE)] / pivot);
        }
      }
      ...
      if (k + 1 < kend) {
        ...
        for (jj = kend; jj < SIZE; jj+= NC) {
          ...
          UpdateKernel(ib, jb, &A[IDX(k+1, jj, SIZE)], SIZE, packed_pivot, packed_mult_panel);
        }
      }
    }
