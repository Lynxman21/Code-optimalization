...
int ge(double *A, int SIZE)
{
  ...
  double *packed_mult_all = (double*) malloc(SIZE*sizeof(double));
  if (packed_mult_all == NULL) {
    return -1;
  }

  for (k = 0; k < SIZE; k++) {
    double pivot = A[IDX(k, k, SIZE)];
    PackMultipliers(SIZE - k - 1, A, SIZE, k + 1, k, pivot, packed_mult_all);
    ...
    for (jj = k + 1; jj < SIZE; jj += NC) {
      ...
      for (ii = k + 1; ii < SIZE; ii += MC) {
        ...
        double *mult = packed_mult_all + (ii -( k + 1));
        UpdateKernel(ib, jb, &A[IDX(ii, jj, SIZE)], SIZE, packed_pivot, mult);
      }
    }
  }

  free(packed_mult_all);
  return 0;
}
